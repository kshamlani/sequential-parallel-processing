...
class Ion_auth_model extends CI_Model
{
    ...
    public function device_registration($data){
        ...
        if($devices){
            foreach ($devices as $device) {
                if($device->user_id != $data->user_id){
                    $this->notifications->de_register_device($device);
                }
                else{
                    $flag = $this->notifications->check_endpoint_exist($device);
                    if(!$flag){
                        $this->notifications->register_device($data);
                    }
                }
            }
        }

        return $this->notifications->register_device($data);
	...
	public function reset_password($identity, $new) {
		...
		if (!$this->identity_check($identity)) {
            ...
            return ['status' => 2, 'key' => 'email_not_exists'];
		...
		if ($return)
		{
            ...
            log_message('dev', '---------Password reset successful');
            $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_reset_password_complete', 'ion_auth'), $data, true);
            $this->email->clear();
            $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
            $this->email->to($identity);
            $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_reset_password_subject'));
            $this->email->message($message);

            if ($this->email->send())
            {
                log_message('dev', '---------Password reset mail sent');
            }
            else
            {
                log_message('dev', '---------Password reset mail failed');
            }

            return ['status' => 1, 'key' => 'password_reset_success'];
		}
		else
		{
			$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
			$this->set_error('password_change_unsuccessful');
            return ['status' => 3, 'key' => 'password_reset_fail'];
		}
     ...
     *
     */
    public function generate_otp($userid = NULL){
        if(!$userid){
            return false;
        }
        ...
        $data['otp'] = generateRandomNumberString($this->config->item('otp_length'));

        if($this->generic_model->addGenericData('auth_users_otp', $data)){
            $this->load->model('user_model');
            $this->user_model->id = $userid;
            $user = $this->user_model->get();

            $this->load->library('twilio');
            $this->twilio->toSend = str_replace('-', '', $user[0]->phone_number);
            $this->twilio->message = $data['otp'];
            if($this->twilio->send()){
                return TRUE;
            }
            return FALSE;
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($query->num_rows() !== 1)
		{
			$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
			$this->set_error('password_change_unsuccessful');
			return ['status' => 3, 'key' => 'email_not_exists'];
			//return FALSE;
		}
		...
		if ($old_password_matches === TRUE)
		{
			...
			if ($successfully_changed_password_in_db)
			{
                ...
                log_message('dev', '---------Password changed successful');
                $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_change_password_complete', 'ion_auth'), $data, true);
                $this->email->clear();
                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                $this->email->to($identity);
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_change_password_subject'));
                $this->email->message($message);

                if ($this->email->send())
                {
                    log_message('dev', '---------Password changed mail sent');
                }
                else
                {
                    log_message('dev', '---------Password changed mail failed');
                }

				return ['status' => 1, 'key' => 'password_changed_success'];
			}
			else
			{
				$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
				$this->set_error('password_change_unsuccessful');
				return ['status' => 2, 'key' => 'fail'];
			}

			return $successfully_changed_password_in_db;
		}
		else{
		    return ['status' => 4, 'key' => 'old_pwd_not_matched'];
        }
     ...
     * @author Mathew
     **/
    public function mobile_identity_check($phone_number = '')
    {
        $this->trigger_events('mobile_identity_check');
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_check($identity))
		{
			...
			return ['status' => 1, 'id' => FALSE, 'key' => 'email_already_exists']; //If duplicate email is found
			//return FALSE;
		}
		...
		elseif($this->mobile_identity_check($additional_data['phone_number'])){
		    return ['status' => 2, 'id' => FALSE, 'key' => 'phone_already_exists']; //If duplicate mobile number is found
        }
		...
		if( !isset($query->id) && empty($groups) )
		{
			...
			return ['status' => 3, 'id' => FALSE, 'key' => 'user_group_invalid']; //Invalid default group
			//return FALSE;
		}
		...
		$data = array(
		    ...
		    'created_on' => time_now(),
		    'status'     => ($manual_activation === false ? 2 : 1)
		);
		...
		return (isset($id)) ? ['status' => 5, 'id' => $id, 'key' => 'success'] : ['status' => 4, 'id' => FALSE, 'key' => 'account_creation_fail'];
	}
	...
	public function login($identity, $password, $group_id, $remember=FALSE)
	{
		...
		if (empty($identity) || empty($password))
		{
			...
			return ['status' => 2, 'key' => 'login_fail'];
		}

		$this->trigger_events('extra_where');

		$query = $this->db->select($this->identity_column . ', email, '.$this->tables['users'].'.id, password, status, last_login, group_id, is_phone_verified, phone_number')
                            ->join($this->tables['users_groups'], $this->tables['users_groups'].".user_id = ".$this->tables['users'].".id", 'LEFT')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if($this->is_time_locked_out($identity))
		{
            ...
            //account time locked out
            return ['status' => 3, 'key' => 'login_time_locked_out'];
		}
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->status == "inactive" || $user->status == "blocked")
				{
                    ...
                    return ['status' => 4, 'key' => 'login_user_'.$user->status];
				}
                ...
                if ($user->group_id != $group_id)
                {
                    ...
                    return ['status' => 6, 'key' => 'login_with_different_group'];
                }
                ...
                if (!$user->is_phone_verified)
                {
                    $this->trigger_events('post_login_unsuccessful');
                    //User is either inactive or blocked by admin
                    return ['status' => 7, 'key' => 'otp_not_verified', 'id' => $user->id, 'phone'=> $user->phone_number];
                }
				...
				//Login Successful
                return ['status' => 1, 'key' => 'success', 'id' => $user->id];
			}
			else{
                $this->increase_login_attempts($identity);
			    //Wrong password mismatched
                return ['status' => 5, 'key' => 'login_wrong_password'];
            }
		}
        ...
        return ['status' => 5, 'key' => 'email_not_exists'];
	}
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			return ['status' => 2, 'key' => 'email_already_exists'];
		}

        if (array_key_exists('phone_number', $data) && $this->mobile_identity_check($data['phone_number']) && $user->phone_number !== $data['phone_number'])
        {
            echo "in phone_number";
            $this->db->trans_rollback();

            return ['status' => 3, 'key' => 'phone_already_exists'];
        }
