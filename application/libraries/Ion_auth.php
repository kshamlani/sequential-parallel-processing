...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
      ...
      $user = $this->where($identifier, $identity)->where('status', 'active')->users()->row();  // changed to get_user_by_identity from email
            if($user->login_type != 'general'){
                return ['status' => 5, 'key' => 'different_login_type'];
            }
			if ($user)
			{
				$data = array(
				...
				if(!$this->config->item('use_ci_email', 'ion_auth'))
				{
                    ...
                    return ['status' => 2, 'key' => 'forgot_password_email_failed'];
				}
				else
				{
					...
					{
						$this->set_message('forgot_password_successful');
						return ['status' => 1, 'key' => 'forgot_password_check_email'];
					}
					else
					{
			...
			{
				$this->set_error('forgot_password_unsuccessful');
                return ['status' => 3, 'key' => 'account_not_active'];
			}
		}
		else
		{
			$this->set_error('forgot_password_unsuccessful');
            return ['status' => 4, 'key' => 'email_not_exists'];
		}
	}
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$status = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $group_ids);
			if ($status['id'] !== FALSE)
			{
                ...
                log_message('dev', '------------Profile creation success-----------');
                $additionalData['email'] = $email;
                $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_welcome', 'ion_auth'), $additionalData, true);
                $this->email->clear();
                $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                $this->email->to($email);
                $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_welcome_subject'));
                $this->email->message($message);

                if ($this->email->send())
                {
                    log_message('dev', '------------Welcome mail sent-----------');
                }
                else
                {
                    log_message('dev', '------------Welcome mail failed------------');
                }

				return $status; //Account create successful(5)
				//return $id;
			}
			else
			{
                ...
                return $status; //Account creation fail
			}
		}
		else
		{
